...
extern ULONG Exists[16];										// Successful initialized axes

extern USHORT OEM_Adr  [16][2];
extern USHORT CORR_Adr [16][2];
...
//////////////////////////////////////////////////////////////////////
// Windows95
// IKFind: Returns port address(es) of installed card('s) in pBuffer16
//////////////////////////////////////////////////////////////////////

BOOL IKFind (ULONG* pBuffer16)
{
USHORT	RegW;					// Register value word
USHORT	Ax;

	if (!pBuffer16)																// Check buffer address
	{
	 	DLLStatus = DLLStatus | DLLStatus_BufferAddressInvalid;	return FALSE;	// Buffer address invalid!
	}
	...
	for (Ax=0; Ax<16; Ax++)
	{
		if (PortBase[Ax])
		{
			...
			if (RegW == 0x0007)
				*(pBuffer16+Ax)=(ULONG)PortBase[Ax];							// Return address of IK 220
			else 
				*(pBuffer16+Ax)=0;												// Clear address entry
		}
		else
			*(pBuffer16+Ax)=0;													// Clear address entry
	}
...
BOOL IKFind (ULONG* pBuffer16)
{
	...
	if  (!DeviceIoControl	( m_hIK220Drv,					// Handle to device driver
							  ...
							  pBuffer16,					// Pointer to input buffer
							  16*4,							// Input buffer size
							  pBuffer16,					// Pointer to output buffer
							  16*4,							// Output buffer size
							  &byteCount,					// Bytes returned
							  NULL ))						// Not overlapped
	{
	 	DLLStatus = DLLStatus | DLLStatus_ErrDevIoControl;			return FALSE;	// Error device i/o control!
