...
begin
  ...
  a:=cnt^.count[axis]^.yde_e.off0;
  r:=a;
  str(r:5:0,s);
  drawer.eoff0.Caption:=s;

  a:=cnt^.count[axis]^.yde_e.off9;
  r:=a;
  str(r:5:0,s);
  drawer.eoff9.Caption:=s;

  b:=cnt^.count[axis]^.yde_e.pha1;
  r:=b;
  str(r:5:0,s);
  drawer.ephas.Caption:=s;

  b:=cnt^.count[axis]^.yde_e.sym1;
  r:=b;
  str(r:5:0,s);
...
begin
  ...
  b:=cnt^.count[axis]^.yde_c.pha0;
   if b=0 then
    drawer.esphas.Caption:='+'
  else
    drawer.esphas.Caption:='-';

  b:=cnt^.count[axis]^.yde_c.sym0;
  if b=0 then
    drawer.essym.Caption:='+'
  else
    drawer.essym.Caption:='-';

  b:=cnt^.count[axis]^.yde_c.pha1;
  r:=b;
  r:=0.0005493247*r; (*°*)
  str(r:5:3,s); 
  drawer.cphas.Caption:=s;

  b:=cnt^.count[axis]^.yde_c.sym1;
