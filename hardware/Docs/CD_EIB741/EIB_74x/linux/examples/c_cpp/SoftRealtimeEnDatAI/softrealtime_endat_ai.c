...
#ifdef _WIN32
#define POS_SPEC "Trg-Cntr: %05u, Status: 0x%04X, Pos: %010I64d, AI1: 0x%04X  %d"
#else
#define POS_SPEC "Trg-Cntr: %05u, Status: 0x%04X, Pos: %010lld, AI1: 0x%04X  %d"
#endif
...
struct EncData
{
   ...
   unsigned short ai1_status;							/* additional information 1	   */
   unsigned short ai1_data;								/* additional information 1	   */
};
...
/* register console handler for program termination on user request */
   ...
   CheckError(EIB7SetTimestamp(axis[enc_axis], EIB7_MD_Enable));
	 ...
	 /* enable additinal information 1 */
	 CheckError(EIB7EnDat22SetAddInfo(axis[enc_axis], EIB7_AI1_Diagnostic, EIB7_AI2_Stop));
   ...
   CheckError(EIB7AddDataPacketSection(packet, 1, region, EIB7_PDF_StatusWord | EIB7_PDF_PositionData | EIB7_PDF_Timestamp | EIB7_PDF_EnDat_AI1));
   ...
   while(!stop)
   {
      ...
      if(entries > 0)
      {
         ...
         /* read AI1 value from data packet */
         CheckError(EIB7GetDataFieldPtr(eib, udp_data, enc_axis + 1,
                    EIB7_PDF_EnDat_AI1, &field, &sz));
         EncoderData.ai1_status = (unsigned short)((*(unsigned long*)field) & 0xFFFF);
				 EncoderData.ai1_data = (unsigned short)(((*(unsigned long*)field) >> 16) & 0xFFFF);
				 
         /* print status word and position value */
         printf(POS_SPEC, EncoderData.TriggerCounter, EncoderData.status, EncoderData.position,
         										EncoderData.ai1_status, EncoderData.ai1_data);
         printf("\n");
