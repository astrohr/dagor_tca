...
* 
*  \brief   sample for soft realtime mode with callback function
*  
*  Content:
...
*  by an external trigger signal. If data is available
...
/* Softrealtime demo program
   ...
   timestamp and the trigger counter are displayed from the 
   callback function.
   */
...
/* register console handler for program termination on user request */
   ...
   CheckError(EIB7AddDataPacketSection(packet, 1, region, EIB7_PDF_StatusWord | EIB7_PDF_PositionData | EIB7_PDF_Timestamp | EIB7_PDF_ReferencePos));
   ...
   CheckError(EIB7SetDataCallback(eib, &enc_axis, EIB7_MD_Enable, 1, DataCallback));
   ...
   /* unregister callback */
   CheckError(EIB7SetDataCallback(eib, NULL, EIB7_MD_Disable, 1, NULL));
...
/* Callback for reading data
   The callback function will be called every time when new data arrived. 
   The function reads the data from the FIFO and prints the position value.
   */
void DataCallback(EIB7_HANDLE eib, unsigned long cnt, void* data)
{
   ...
   if(entries > 0)
   {
      ...
      CheckError(EIB7GetDataFieldPtr(eib, udp_data, (EIB7_DataRegion)(enc_axis + 1),
