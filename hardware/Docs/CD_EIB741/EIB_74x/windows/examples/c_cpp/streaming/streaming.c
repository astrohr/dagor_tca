...
* 
*  \brief   sample for streaming mode
*  
*  Content:
*  Sample programm for the streaming mode of
*  the EIB. The program configures one axis
*  of the EIB for incremental encoders and
*  enables the streaming mode. The EIB can
...
#else
#define POS_SPEC "Trg-Cntr: %05u, Timestamp: %010ul, Status: 0x%04X, Pos: %010lld"
#endif
...
#define MAX_SRT_DATA      200    /* maximum size of streaming data    */
...
/* struct for streaming mode data */
struct EncData
{
...
/* Streaming demo program
   This program demonstrates the streaming mode of the EIB.
...
#ifdef _WIN32          /* wait for 5 ms to minmize processor load */
      Sleep(5);
#else
      usleep(5000);
#endif
