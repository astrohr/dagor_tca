...
Usage:
    dome.py status
    dome.py switch <N> (on|off)
    dome.py switch all (on|off)
    dome.py reset
    dome.py -h | --help
    dome.py --version
...
class SwitchController(object):
    ...
    def pretty_status(self):
        ...
        for i in range(self._n):
            print("switch {}: {}".format(i + 1, "ON" if self._status[i] else "OFF"))
    ...
    def _refresh_status(self):
        ...
        if response != 'status':
            raise CommunicationException('Controller doesnt acknowlege status command, instead got: {}'.format(response))
        response = self._serial.readline().strip()
        try:
